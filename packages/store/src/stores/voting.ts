...
class VotingDbStore implements IStoreVoting {
  ...
  async createMessage(message: MessageTaskType){
    ...
    const squadUsers = (await this.#client
      .select('user')
      .where({squad: message.squad, enabled: true})
      .from('squads-users')).map((u)=> u.user)
  ...
  async findTasksPoints(squad: string, task: string){
    return this.#client
    .select('tasks.id as tasks', 'tasks-points.user', 'tasks-points.currentRound', 'tasks.maxRounds', 'tasks.percentual as minPercentual', 'tasks-points.points')
    .where({squad, "tasks.id": task, "tasks.enabled":true, "tasks.active": true, "tasks.finished": false})
    .from('tasks')
    .join('tasks-points', 'tasks-points.task', '=', 'tasks.id')
...
function getAllPercentual (tasks: any, currentRound: Number, allUsers: number){
  const pointsCurrentRound = tasks.map((t: any) => {
    if(t.currentRound === currentRound){
      return t.points
    }
  }).filter((f: Number)=> f !== undefined)
...
function getMaxPercentual (frequency: Record<string, Number>){
  const sorted = Object.values(frequency).sort(function(a: any, b: any){return b-a})
  const maxPercentual = sorted[0] 
  const newPoint = Object.keys(frequency).find((k) => frequency[k] === maxPercentual)
...
function getEstimative (tasks: any, currentRound:Number, frequencyArray: Record<string, number>) : Number{
  ...
  const points = Object.keys(frequencyArray).map((k)=> Number(k))
  ...
  const sum = pointsCurrentRound.reduce((acm: any, s: any) => {return s + acm})
  const media = Math.ceil((maxPoint + (sum/total))/2)

  if (fibonacci.includes(media)){
    newFibonacci = fibonacci.find((f)=> f == media) ?? newFibonacci
  } else{
    const idx = fibonacci.findIndex((f, idx)=>{
