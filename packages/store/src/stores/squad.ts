...
class SquadDbStore implements IStoreSquad {
  ...
  async addSquadUsersByEmail(squadId: string, email: string, owner: boolean): Promise<SquadUsersType | void> {
    const [userDb, squadDb] = await Promise.all([this.#client('users').select('name', 'id', 'email').where({ email, enabled: true }), this.#client('squads').select('name', 'id').where({ id: squadId, enabled: true })]);
    ...
    if (userDb.length !== 0 && squadDb.length !== 0 && !alreadyExist) {
      await this.#client('squads-users').insert({ id: randomUUID(), user: userDb[0].id, squad: squadId, enabled: owner });
  ...
  async find(squadId: string): Promise<LoadedSquadsByUserIdType | void> {
    const res = await this.#client.select('squads.id as squad', 'squads.name as squadName', 'squads.currentMaxRounds', 'squads.currentPercentual', 'squads.updatedAt', 'users.id as user', 'users.name as userName', 'users.email as email').where({ squad: squadId }).from('squads-users').leftJoin('users', 'users.id', '=', 'squads-users.user').leftJoin('squads', 'squads.id', '=', 'squads-users.squad').where({ 'users.enabled': true, 'squads-users.enabled': true });
